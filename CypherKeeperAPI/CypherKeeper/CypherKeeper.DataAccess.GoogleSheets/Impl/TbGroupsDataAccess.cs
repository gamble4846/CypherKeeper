...
namespace CypherKeeper.DataAccess.GoogleSheets.Impl
{
    public class TbGroupsDataAccess : ITbGroupsDataAccess
    {
        ...
        public TbGroupsDataAccess(string sheetId, CommonFunctions _cf)
        {
            try
            {
                SheetId = sheetId;
        ...
        public List<tbGroupsModel> Get(int page = 1, int itemsPerPage = 100, bool onlyNonDeleted = true)
        {
            var FinalData = _GoogleSheetsCrud.GetList<tbGroupsModel>(-1, -1);
            if (onlyNonDeleted)
            {
        ...
        {
            model.Id = Id;
            var rowNumber = _GoogleSheetsCrud.GetRowNumbers<tbGroupsModel>("Id", Id.ToString());
        ...
        public int GetTotalByParentGroupId(Guid? ParentGroupId, bool onlyNonDeleted = true)
        {
            ...
            return FinalData.Count(x => x.ParentGroupId == ParentGroupId);
        }
    }
}
